...
try:
    ...
    API_KEY = 'car2gowebsite'  # default key used by car2go.com
...
for city, city_data in CITIES.items():
    city_data['API_AVAILABLE_VEHICLES_URL'] = API_URL.format(key=API_KEY, loc=city)
...
def get_operation_areas(city):
    ...
    r = requests.get(API_AREAS_URL.format(loc=city, key=API_KEY))
...
def get_parking_spots(city):
    ...
    r = requests.get(API_PARKING_URL.format(loc=city, key=API_KEY))
